...
const MyDrawer: React.FC<MyDrawerProps> = props => {
    ...
    const handleStartScanImages = () => {
        handleCloseMenu()
        props.startScanAction(props.token, 'image')
    }

    const handleStartScanVideos = () => {
        handleCloseMenu()
        props.startScanAction(props.token, 'video')
    }
    ...
    let drawerAlbums = props.currentModule === Module.Image ? (
        <DrawerAlbumsImage />
    ) : undefined
...
export default withSize({ monitorHeight: true })(connect(
    mapStateToProps,
    { startPhotosFetched, startVideosFetched, selectPhoto, selectNextPhoto, selectPreviousPhoto,
